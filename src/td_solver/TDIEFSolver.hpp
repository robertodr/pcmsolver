...
private:
  ...
  virtual void buildSystemMatrix_impl(
      ...
      const BoundaryIntegralOperator & op) __override;
  /*! \brief Calculation of the PCM matrix, using Lowdin symmetric orthogonalization
   *  \param[in] cavity the cavity to be used
   *  \param[in] gf_i   Green's function inside the cavity
   *  \param[in] op integrator strategy for the single and double layer operators
   */
  void systemMatrix_Lowdin(const Cavity & cavity, const IGreensFunction & gf_i,
   ...
   *  \param[in] op integrator strategy for the single and double layer operators
   */
  void systemMatrix_Cholesky(const Cavity & cavity, const IGreensFunction & gf_i,
                             const BoundaryIntegralOperator & op);
  ...
  virtual Eigen::VectorXd propagateASC_impl(
      double dt, const Eigen::VectorXd & MEP_current,
