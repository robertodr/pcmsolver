...
double pcmsolver_propagate_asc(pcmsolver_context_t * context, const char * mep_t,
                               const char * asc_t, const char * mep_tdt,
                               const char * asc_tdt, double dt, int irrep) {
...
namespace pcm {
Meddle::Meddle(const Input & parsed)
    : input_(parsed), hasDynamic_(false), hasTD_(false) {
...
void Meddle::initializePropagation(const char * mep_0, const char * asc_0,
                                   const char * mep_t, const char * asc_t,
                                   const char * mep_tdt, const char * asc_tdt,
                                   int irrep) const {
...
double Meddle::propagateASC(const char * mep_t, const char * asc_t,
                            const char * mep_tdt, const char * asc_tdt, double dt,
                            int irrep) const {
  double energy = 0.0;
  if (input_.isTD()) {
...
double Meddle::delayedASC(const char * mep_t, const char * asc_t,
                          const char * mep_tdt, const char * asc_tdt, double dt,
                          int /* irrep */) const {
  std::string MEP_current(mep_tdt);
  std::string MEP_previous(mep_t);
...
void Meddle::initTDSolver() {
  TD_K_ = Factory<TDPCMSolver, TDSolverData>::TheFactory().create(
      input_.TDsolverType(), input_.TDSolverParams());
  BoundaryIntegralOperator * biop =
      Factory<BoundaryIntegralOperator, biOperatorData>::TheFactory().create(
          input_.integratorType(), input_.integratorParams());
  IGreensFunction * gf_i = Factory<IGreensFunction, greenData>::TheFactory().create(
