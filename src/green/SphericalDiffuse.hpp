...
public:
   ...
   */
  SphericalDiffuse(double e1, double e2, double w, double c,
                   const Eigen::Vector3d & o, int l, double fac)
      : GreensFunction<Numerical, ProfilePolicy>(fac),
        origin_(o),
        maxLGreen_(l),
        maxLC_(2 * l) {
    initProfilePolicy(e1, e2, w, c);
  ...
  double coefficientCoulombDerivative(const Eigen::Vector3d & direction,
    ...
    return threePointStencil(
        pcm::bind(&SphericalDiffuse<ProfilePolicy>::coefficientCoulomb, this,
                  pcm::_1, pcm::_2),
  ...
  EIGEN_MAKE_ALIGNED_OPERATOR_NEW /* See
       ...
       */
      virtual Numerical
      operator()(Numerical * sp, Numerical * pp) const __override {
  ...
  virtual double singleLayer_impl(const Element & e) const __override {
    ...
    double Sii_I = integrator::diagonalSi(e.area(), this->factor_);
  ...
  virtual double doubleLayer_impl(const Element & e) const __override {
    ...
    double Dii_I =
        integrator::diagonalDi(e.area(), e.sphere().radius, this->factor_);
    // "Diagonal" of Coulomb singularity separation coefficient
    double coulomb_coeff = this->coefficientCoulomb(e.center(), e.center());
  ...
  void initSphericalDiffuse() {
    ...
    double r_infinity_ = this->profile_.center() +
