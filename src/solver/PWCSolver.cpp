...
static double DoubleLayer(Vector3 x, Vector3 y, Vector3 n_y)
{
    ...
    double value = gf->kernelD(vn_y, vx, vy);
    return value;
}
...
void PWCSolver::buildSystemMatrix_impl(const Cavity & cavity, const IGreensFunction & gf_i, const IGreensFunction & gf_o)
{
    // Dirty hack to have the permittivity available deep down in the solver
    epsilon_ = gf_o.permittivity();
    ...
    try {
        ...
        constructSystemMatrix(gf_i, gf_o);
        timerOFF("SystemMatrix");
    } catch (const std::bad_cast & e) {
        PCMSOLVER_ERROR(e.what() +
                                 std::string(" Wavelet type cavity needed for wavelet solver."));
    }
    // Set systemMatrixInitialized_ and built_ to true
    systemMatrixInitialized_ = true;
    built_ = true;
}
...
void PWCSolver::constructSystemMatrix(const IGreensFunction & gf_i, const IGreensFunction & gf_o)
{
    constructSi(gf_i);
...
void PWCSolver::constructSi(const IGreensFunction & gf_i){
    double factor = 0.0;
    switch (integralEquation) {
        case FirstKind:
            factor = - 2 * M_PI * (epsilon_ + 1) / (epsilon_ - 1);
            break;
        case SecondKind:
            factor = - 2 * M_PI * (epsilon_ + 1) / (epsilon_ - 1);
            //PCMSOLVER_ERROR("Second Kind not yet implemented"); //careful to the double layer sign when implementing it....
            break;
        case Full:
            factor = 2 * M_PI;
            break;
        default:
            PCMSOLVER_ERROR("Unknown integral equation type.");
    }
    gf = &gf_i;
    timerON("compression");
...
void PWCSolver::constructSe(const IGreensFunction & gf_o){
    gf = &gf_o; // sets the global pointer to pass GF to C code
    apriori2_ = af->compression(&S_e_);
    WEM(af, &S_e_, SingleLayer, DoubleLayer, -2*M_PI);
    aposteriori2_ = af->postProc(&S_e_);
}

Eigen::VectorXd PWCSolver::computeCharge_impl(const Eigen::VectorXd & potential,
                           int /* irrep */)
{
    Eigen::VectorXd charge = Eigen:VectorXd::Zero(potential.size());
    switch (integralEquation) {
    case FirstKind:
