...
namespace
{
    Cavity* createWaveletCavity(const cavityData & data)
    {
        return new WaveletCavity(data.molecule.spheres(), data.probeRadius, data.patchLevel,
                data.coarsity);
    }
    const std::string WAVELET("WAVELET");
    const bool registeredWavelet = Factory<Cavity, cavityData>::TheFactory().registerObject(
