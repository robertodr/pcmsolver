 ...
 */
typedef pcm::function<double(const Eigen::Vector3d &, const Eigen::Vector3d &)> GFValue;

/*! \typedef GFDerivative
 *  \brief functor handle to the derivative of a Green's function in a point
 */
typedef pcm::function<double(const Eigen::Vector3d &, const Eigen::Vector3d &, const Eigen::Vector3d &)> GFDerivative;
 ...
 *  \return the Newton potential on the grid
 */
Eigen::VectorXd computeDipolarPotential(const GFDerivative & gf, const Eigen::Matrix3Xd & grid, const ChargeDistribution & dist);
