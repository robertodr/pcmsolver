...
//      return eps_r2 * (Dii_I / coulomb_coeff - Sii_I * coeff_grad + image_grad);
    ...
    /**@{ Single and double layer potentials for a PlanarDiffuse Green's function by collocation: numerical integration of diagonal */
    template <typename ProfilePolicy>
    Eigen::MatrixXd singleLayer(const PlanarDiffuse<NumericalIntegrator, ProfilePolicy> & /* gf */, const std::vector<Element> & e) const {
//      // The singular part is "integrated" as usual, while the nonsingular part is evaluated in full
//      double area = e.area();
//      // Diagonal of S inside the cavity
//      double Sii_I = factor_ * std::sqrt(4 * M_PI / area);
...
//      return (Sii_I / coulomb_coeff + image);
    ...
    template <typename ProfilePolicy>
    Eigen::MatrixXd doubleLayer(const PlanarDiffuse<NumericalIntegrator, ProfilePolicy> & /* gf */, const std::vector<Element> & e) const {
//      // The singular part is "integrated" as usual, while the nonsingular part is evaluated in full
//      double area = e.area();
//      double radius = e.sphere().radius();
//      // Diagonal of S inside the cavity
