 ...
 */
TEST_CASE("Input reading using GetKw for an input file for a classical point multipoles distribution", "[input][multipoles][input_multipoles]")
{
  ...
  for (size_t j = 0; j < 3; ++j) {
    ...
    REQUIRE(Eigen::Vector3d::UnitZ()(j) == Approx(parsedInput.multipoles().dipoles(j, 0)));
  }
}
