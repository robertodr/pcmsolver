...
/*! \class VIEFSolver
 *  \test \b C2H4D2hGePol tests ASC error vector for VIEFSolver using C2H4 in D2h and a GePol cavity
 */
TEST_CASE("Test variational solver ASC error vector for the IEFPCM with C2H4 molecule and a GePol cavity",
          "[variational_solver][error][viefpcm][viefpcm-error_gepol-C2H4_D2h]")
{
  Molecule molec = C2H4();
  double area = 10.0;
  ...
  for (size_t i = 0; i < size; ++i) {
    ...
    // We have to transform to the dressed representation
    Eigen::VectorXd fake_mep_dressed = solver.getDressedMEP(fake_mep);
    REQUIRE(error(i) == Approx(fake_mep_dressed(i)));
  }
}
