...
SCENARIO(
    ...
    "[td_solver][tdcpcm][tdcpcm_gepol-point]") {
  GIVEN("An isotropic environment modelled as a perfect conductor and a point "
    ...
    /*! \class TDCPCMSolver
     *  \test \b pointDipoleGePol tests TDCPCMSolver using a point dipole with a
     * GePol cavity
     */
    WHEN("the point dipole is located at the origin") {
      ...
      solver.buildSystemMatrix(cavity, gfInside, S);

      bool symm = true;
      CPCMSolver static_solver(symm, correction);
      static_solver.buildSystemMatrix(cavity, gfInside, gfOutside, S);
