 ...
 *  \test \b pointDipoleGePol tests TDCPCMIterativeSolver using a point dipole with a GePol cavity
 */
TEST_CASE("Test solver for the time-dependent CPCM iterative solver with a point dipole and a GePol cavity", "[td_solver][tdcpcm][tdcpcm-iter_gepol-point]")
{
    // Set up cavity
    ...
    TDCPCMIterativeSolver<> solver(e_0, e_d, tau, correction);
    ...
    cnpy::NpyArray raw_TDCPCM_ref = cnpy::npy_load("tdcpcmiterative_collocation.npy");
