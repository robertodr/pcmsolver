 ...
 *  \test \b pointDipoleGePol tests TDSingleIEFSolver using a point dipole with a GePol cavity
 */
TEST_CASE("Test solver for the time-dependent IEFPCM with a single relaxation time, a point dipole and a GePol cavity", "[td_solver][tdief][tdsingleief_gepol-point]")
{
    // Set up cavity
    ...
    std::vector<double> TDSingleIEF;
    TDSingleIEF.reserve(steps);
    Eigen::VectorXd asc = Eigen::VectorXd::Zero(size);
    for (int i = 0; i < steps; ++i) {
        TDSingleIEF.push_back(-fake_mep.dot(ASC_previous));
        asc = solver.propagateASC(dt, fake_mep, fake_mep, ASC_previous);
        ASC_previous = asc;
    }
    cnpy::NpyArray raw_TDSingleIEF_ref = cnpy::npy_load("tdsingleief_collocation.npy");
    double * TDSingleIEF_ref = reinterpret_cast<double *>(raw_TDSingleIEF_ref.data);
    for (int i = 0; i < steps; ++i) {
        REQUIRE(TDSingleIEF[i] == Approx(TDSingleIEF_ref[i]));
    }
}
