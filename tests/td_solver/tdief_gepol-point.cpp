...
SCENARIO("Test time-dependent solver for the IEFPCM for a point dipole and a GePol "
         "cavity",
         "[td_solver][tdiefpcm][tdief_gepol-point]") {
  GIVEN("An isotropic environment and a point dipole") {
    ...
    /*! \class TDIEFSolver
     *  \test \b pointDipoleGePolLowdin tests TDIEFSolver using a point dipole with a
     * GePol cavity
    ...
    WHEN("the point dipole is located at the origin") {
      ...
      THEN("the elements of the Lambda matrix are") {
        Eigen::VectorXd Lambda_ref =
            cnpy::custom::npy_load<double>("tdief_lowdin_collocation_Lambda.npy");
        for (size_t i = 0; i < size; ++i) {
          REQUIRE(solver.Lambda(i) == Approx(Lambda_ref(i)));
        }
      }
      AND_THEN("the elements of the K_0 matrix are") {
        Eigen::VectorXd K_0_ref =
            cnpy::custom::npy_load<double>("tdief_lowdin_collocation_K_0.npy");
        for (size_t i = 0; i < size; ++i) {
          REQUIRE(solver.K_0(i) == Approx(K_0_ref(i)));
        }
      }
      AND_THEN("the elements of the K_d matrix are") {
        Eigen::VectorXd K_d_ref =
            cnpy::custom::npy_load<double>("tdief_lowdin_collocation_K_d.npy");
        for (size_t i = 0; i < size; ++i) {
      ...
      AND_THEN("the elements of the tau matrix are") {
        Eigen::VectorXd tau_ref =
            cnpy::custom::npy_load<double>("tdief_lowdin_collocation_tau.npy");
        for (size_t i = 0; i < size; ++i) {
      ...
      AND_THEN("the ASC trajectory is") {
        cnpy::NpyArray raw_TDIEF_ref =
            cnpy::npy_load("tdief_lowdin_collocation.npy");
